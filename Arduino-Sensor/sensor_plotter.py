...
def plot_all_sensors_together(data, filename):
    ...
    dest = f'graphs/{foldername}/{out}'
    fig.savefig(dest, dpi=150, bbox_inches='tight', show=False)
    print(f'  Saved: {dest}')
    #plot.close()
    #plt.show()
...
def plot_individual_sensors(data, filename):
    ...
    for sensor_id in range(1, 6):
        ...
        fig.savefig(out, dpi=150, bbox_inches='tight', show=False)
        print(f'  Saved: {out}')
        #plot.close()
        #plt.show()
...
def main():
    ...
    foldername = filename.removesuffix('.txt')
    os.mkdir(f'graphs/{foldername}')
