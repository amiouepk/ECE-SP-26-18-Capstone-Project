...
unsigned long queueInterval = 20;  // Default 50 Hz (Adjustable via Serial)
...
// ==========================================
// CORE 0: NETWORKING TASK (FreeRTOS)
// ==========================================
...
void networkTask(void *pvParameters) {+
  ...
  Serial.println("Core 0: WiFi & WebSockets Ready");
  ...
  for (;;) {
    ...
    if (activeClients > 0) {
      ...
      if (xSemaphoreTake(queueMutex, portMAX_DELAY) == pdTRUE) {
        ...
        xSemaphoreGive(queueMutex); // FreeRTOS Mutex Give
      }
    ...
    // Crucial: Feed the FreeRTOS watchdog timer so the network stack breathes
    vTaskDelay(pdMS_TO_TICKS(1)); 
  }
}

// ==========================================
// CORE 1: HARDWARE, MATH & PRODUCER
// ==========================================
void setup() {
  Serial.begin(115200);
  delay(2000); 
  ...
  // Create the Network Task and pin it to Core 0
...
void loop() {
  ...
  if (now - lastQueueTime >= queueInterval) {
    ...
    if (activeClients > 0) {
      ...
      // Format BNOs (Comma Separated)
      for (uint8_t bno=0; bno<BNOs; bno++) {
        if (syncMsg.length() > 0) syncMsg += ","; 
        
        syncMsg += String(kACC*iax[bno], 3) + "," + String(-kACC*iay[bno], 3) + "," + String(-kACC*iaz[bno], 3) + "," +
      ...
      // Format MPUs (Comma Separated)
      for (uint8_t i=0; i<MPUs; i++) {
        if (syncMsg.length() > 0) syncMsg += ","; 
        
        syncMsg += String(mpu_a[i].acceleration.x / 9.80665, 3) + "," + String(mpu_a[i].acceleration.y / 9.80665, 3) + "," + String(mpu_a[i].acceleration.z / 9.80665, 3) + "," +
